...
def compute_type_advantage(mon1, mon2):
    ...
    for type_ in mon1.types:
        if type_:
            a_on_b = max(a_on_b, type_.damage_multiplier(*mon2.types))

    # Do the other way around
    for type_ in mon2.types:
        if type_:
            b_on_a = max(b_on_a, type_.damage_multiplier(*mon1.types))
...
def compute_effective_speed(battle, mon):
    ...
    # Abilities
    if battle.weather:
        if mon.ability == "Slush Rush" and battle.weather.name == "HAIL":
            speed *= 2
        elif mon.ability == "Sand Rush" and battle.weather.name == "SANDSTORM":
            speed *= 2
        elif mon.ability == "Chlorophyll" and battle.weather.name in [
            "SUNNYDAY",
            "DESOLATELAND",
        ]:
            speed *= 2
        elif mon.ability == "Swift Swim" and battle.weather.name in [
            "RAINDANCE",
            "PRIMORDIALSEA",
        ]:
            speed *= 2
        elif mon.ability == "Surge Surfer" and Field.ELECTRIC_TERRAIN in battle.fields:
            speed *= 2  # Field(1) corresponds to Electric Terrain

    # Incorporate boosts
    speed *= statMod(mon.boosts["spe"])

    # Incorporate status (Paralysis) if the mon doesnt have quickfeed
    if mon.ability == "Quick Feet" and not mon.status:
        speed *= 1.5
    elif mon.status and mon.status == Status.PAR:
        speed *= 0.5  # Status(4) corresponds to paralysis

    # Held Items (choice scarf, iron ball)
    if mon.item == "ironball":
        speed *= 0.5
    elif mon.item == "choicescarf":
        speed *= 1.5

    # Side Conditions
    if SideCondition.GRASS_PLEDGE in battle.side_conditions:
        speed *= 0.25  # GRASS_PLEDGE, or creating a swamp
    elif SideCondition.TAILWIND in battle.side_conditions:
        speed *= 2  # TAILWIND
...
def compute_worst_case_scenario_speed(battle, mon):
    ...
    speed *= 0.9 if tr else 1.1

    # Abilities; assuming that a mon would use their speed abilities
    if battle.weather:
        if "Slush Rush" in mon.possible_abilities and battle.weather.name == "HAIL":
            speed *= 2
        elif (
            "Sand Rush" in mon.possible_abilities and battle.weather.name == "SANDSTORM"
        ):
            speed *= 2
        elif "Chlorophyll" in mon.possible_abilities and battle.weather.name in [
            "SUNNYDAY",
            "DESOLATELAND",
        ]:
            speed *= 2
        elif "Swift Swim" in mon.possible_abilities and battle.weather.name in [
            "RAINDANCE",
            "PRIMORDIALSEA",
        ]:
            speed *= 2
        elif (
            "Surge Surfer" in mon.possible_abilities
...
def battle_debug(battle):
    ...
    for i, mon in enumerate(battle.active_pokemon):
        print(
            f"{i}th mon's moves:",
            ", ".join(map(lambda x: x.id, battle.available_moves[i])),
        )

    for i, mon in enumerate(battle.active_pokemon):
        print(
            f"{i}th mon's available switches:",
            ", ".join(map(lambda x: x.species, battle.available_switches[i])),
        )
